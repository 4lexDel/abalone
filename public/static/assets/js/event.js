...
socket.on("room_joined", (state, player, message) => {
    if (player != null) {
        ...
        if (playerInfo.status == "Player1") canvasObject.playerToPlay = 1;
        else if (playerInfo.status == "Player2") canvasObject.playerToPlay = 2;
        else canvasObject.playerToPlay = -1;
    }
...
socket.on("room_left", () => {
    ...
    canvasObject.resetGrid(canvasObject.coordsGrid); //////////////////
});
...
socket.on("grid_refresh", (grid) => {
    if (grid != null) {
        ...
        if (playerInfo != null) {
            ...
            canvasObject.coordsGrid = grid;

            if (playerInfo.status == "Player2") canvasObject.reverseGrid(canvasObject.coordsGrid);
