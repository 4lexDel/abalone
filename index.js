...
io.on('connection', (socket) => {
    ...
    socket.on("join_room", (room, pseudo) => { //Vire d'une room dans tous les cas ????????
        if (io.sockets.adapter.rooms.has(room)) { //Est ce que la room existe
            ...
            if (roomObj != null) {
                ...
                messageToSocket(socket, "grid_refresh", roomObj.abalone.grid);
    ...
    socket.on("piece_move", (x1, y1, dx, dy) => {
        ...
        if (playerObj != null) {
            ...
            if (playerToPlay != -1) {
                ...
                if (roomObj != null) {
                    let moveResult = roomObj.abalone.play(playerToPlay, parseInt(x1), parseInt(y1), parseInt(dx), parseInt(dy));
                    if (moveResult) {
                        broadcast(socket, roomID, "grid_refresh", roomObj.abalone.grid, true);
    ...
    socket.on("restart_game", () => {
        ...
        if (roomObj != null) {
            roomObj.abalone.resetGame();

            broadcast(socket, playerObj.roomID, "grid_refresh", roomObj.abalone.grid, true);
...
function getRealTimeInformation(roomObj) {
    if (roomObj != null && roomObj.abalone != null) {
        if (roomObj.abalone.winner != -1) {
            handleEndGame(roomObj);
            if (roomObj.abalone.winner == 1) return '<span style="color:black">Player 1 win !</span>';
            else return '<span style="color:red">Player 2 win !</span>';
        } else if (roomObj.abalone.playerToPlay == 2) return '<span style="color:red">Player 2 to play</span>';
        else if (roomObj.abalone.playerToPlay == 1) return '<span style="color:black">Player 1 to play</span>';
    }
